...
RESOLVED_DROP_IN = Path("/etc/systemd/resolved.conf.d/dawos.conf")
...
def set_dns(
    ...
    On systems using systemd-resolved (detected when ``/etc/resolv.conf``
    is a symlink), writes a drop-in at
    ``/etc/systemd/resolved.conf.d/dawos.conf`` and restarts the
    resolver.  Otherwise writes ``/etc/resolv.conf`` directly via
    ``sudo tee``.
    ...
    if resolv_path:
        # Testing path — write resolv.conf directly without sudo
    ...
    # Detect systemd-resolved (resolv.conf is a symlink)
    if RESOLV_CONF.is_symlink():
        _write_resolved_dropin(nameservers, search_domains)
    else:
        _write_resolv_conf(nameservers, search_domains)


def _write_resolved_dropin(
    nameservers: list[str],
    search_domains: list[str] | None = None,
) -> None:
    """Write a systemd-resolved drop-in and restart the service.

    Creates ``/etc/systemd/resolved.conf.d/dawos.conf`` with the
    ``[Resolve]`` section containing ``DNS=`` and ``Domains=`` directives.
    """
    lines = ["# Generated by dawos-agent\n", "[Resolve]\n"]
    lines.append(f"DNS={' '.join(nameservers)}\n")
    if search_domains:
    ...
    result = subprocess.run(  # pylint: disable=subprocess-run-check
        ["sudo", "mkdir", "-p", str(RESOLVED_DROP_IN.parent)],
        stderr=subprocess.PIPE,
    )
    if result.returncode != 0:
        err = result.stderr.decode().strip()
        raise PermissionError(f"Failed to create resolved drop-in dir: {err}")

    result = subprocess.run(  # pylint: disable=subprocess-run-check
        ["sudo", "tee", str(RESOLVED_DROP_IN)],
    ...
    if result.returncode != 0:
        err = result.stderr.decode().strip()
        raise PermissionError(f"Failed to write {RESOLVED_DROP_IN}: {err}")
...
def _write_resolv_conf(
    ...
    """Write ``/etc/resolv.conf`` directly via ``sudo tee``."""
    lines = ["# Generated by dawos-agent\n"]
    if search_domains:
        lines.append(f"search {' '.join(search_domains)}\n")
    for ns in nameservers:
    ...
    if result.returncode != 0:
        err = result.stderr.decode().strip()
        raise PermissionError(f"Failed to write {RESOLV_CONF}: {err}")
