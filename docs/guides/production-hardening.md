...
The 153 API endpoints are function registrations, not running processes. At runtime, only the endpoint being called executes. Each request spawns a single lightweight subprocess (`accel-cmd`, `nft`, `ip`, etc.), collects the output, and returns JSON. The entire cycle typically completes in under 100 milliseconds.
